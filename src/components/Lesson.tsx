...
export function Lesson(props: LessonProps){
    ...
    return(
        <Link to={`/event/lesson/${props.slug}`} className='group'>
            ...
            <div className={`rounded border  p-4 mt-2 group-hover:border-green-500 ${isActiveLesson ? 'bg-green-500 border-white' : 'border-gray-500'}`} > 
                <header className="flex items-center justify-between">
                    {isLessonAvailable ? (
                        ...
                        <span className={`text-sm font-medium flex gap-2 items-center ${isActiveLesson ? 'text-white' : 'text-orange-500'}`}>
                            <Lock size={20}/>
                            Em breve
                        </span>
                    )
